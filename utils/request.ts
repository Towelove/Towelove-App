...
instance.interceptors.request.use((config: InternalAxiosRequestConfig<any>) => {
  ...
  return Promise.all([
    getLocal(tokenName).then((token) => {
      if (token) {
        config.headers.set("Authorization",token as string);
      }
    }),
    getLocal(coupleName).then((coupleId) => {
      if (coupleId) {
        config.headers.set("X-USER-COUPLE", coupleId);
      }
    }),
  ]).then(() => {
    // 两个 Promise 都完成后，返回 config
    console.log(config.headers,'头部')
    return config;
  });
}, errorHandler);
...
// 不走json-bigint渠道
export const requestAsNumber = async (
  url: string,
...
// 文件上传专用渠道
export const fileRequestPost = async (
  url: string,
  data: File | FormData,
  config: AxiosRequestConfig
) => {
